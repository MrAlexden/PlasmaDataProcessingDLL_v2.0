...
#define WORKGROUP_SIZE 256 // needs to be 64 to fully use AMD GPUs
//#define PTX
//#define LOG
//#define USE_OPENCL_1_1

#pragma comment(lib, __FILE__"\\..\\OpenCL\\lib\\OpenCL.lib")
...
inline void print_device_info(const Device_Info& d, const int id=-1) { // print OpenCL device info
	...
	println("| Compute Units  | "+alignl(58, to_string(d.compute_units)+" at "+to_string(d.clock_frequency)+" MHz ("+to_string(d.cores)+" cores, "+
		to_string(d.tflops
#if (FUNCTIONS_AND_CONSTANTS_REDEFINITIONS > 0)
			, 3
#endif
		)+" TFLOPs/s)")+" |");
...
public:
	...
	inline void construct(const Device_Info& info, const string& opencl_c_code = get_opencl_c_code()) // same constructor but for post-initialization
	{
		this->info = info;
		cl_context = cl::Context(info.cl_device);
		cl_queue = cl::CommandQueue(cl_context, info.cl_device); // queue to push commands for the device
...
public:
	...
	template<class... T> 
	inline void construct(const Device& device, const ulong N, const string& name, const T&... parameters) // same constructor but for post-initialization
	{ // accepts Memory<T> objects and fundamental data type constants
		if (!device.is_initialized()) print_error("No Device selected. Call Device constructor.");
		cl_kernel = cl::Kernel(device.get_cl_program(), name.c_str());
		link_parameters(number_of_parameters, parameters...); // expand variadic template to link kernel parameters
	...
	inline cl::Kernel& get_cl_kernel()
	{
		return cl_kernel;
	}
};